/* =========================================================================
   ...
   Changelog:
   + Added pulse_type to all events
   - Removed duplicate UTM/tid fields from form_data
   + Standardised tid_p naming
   ========================================================================= */
...
(function () {
  ...
  function initPulse() {
    ...
    function sendJSON(url, payload){
      ...
      try {
        if (navigator.sendBeacon && navigator.sendBeacon(url, body)) {
          console.info('[Pulse] beacon sent →', url);
          return Promise.resolve({ ok:true, beacon:true });
        }
      } catch (err) {
        console.warn('[Pulse] beacon failed, falling back', err);
      }

      // Fire-and-forget fallback (no-CORS)
      try {
        fetch(url, {
          method:'POST',
        ...
        console.warn('[Pulse] fetch failed silently', err);
      }
    ...
    function serializeForm(form){
      ...
      // remove duplicate tracking fields already in top-level
      ['utm_source','utm_medium','utm_campaign','utm_term','utm_content','tid','tid_p','tidp'].forEach(k => delete out[k]);
